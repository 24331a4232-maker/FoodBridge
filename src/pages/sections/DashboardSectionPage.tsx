...
const loginRoles: { value: 'volunteer' | 'admin'; label: string; icon: typeof Truck; accent: string }[] = [
  { value: 'volunteer', label: 'Volunteer', icon: Truck, accent: 'from-primary-500 to-primary-700' },
  { value: 'admin', label: 'Admin', icon: ShieldCheck, accent: 'from-accent-500 to-accent-700' },
];
  ...
  return (
    <div className="pt-20 min-h-screen gradient-bg-soft relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-20 left-10 h-80 w-80 rounded-full bg-primary-300/10 blur-3xl animate-blob" />
        <div className="absolute bottom-20 right-10 h-80 w-80 rounded-full bg-accent-300/10 blur-3xl animate-blob" style={{ animationDelay: '2s' }} />
      </div>
                ...
                {loginRoles.map((r) => (
                  <motion.button
                    ...
                    className={`p-3 rounded-2xl-premium text-center transition-all ${
                      role === r.value
                        ? `bg-gradient-to-br ${r.accent} text-white shadow-lg`
...
function SignedInHero({ profile, onNavigate }: { profile: NonNullable<ReturnType<typeof useAuth>['profile']>; onNavigate: () => void }) {
  ...
  const stats = [
    { label: 'Reward Points', value: profile.reward_points ?? 0, icon: Zap, color: 'from-yellow-500 to-gold-500' },
    { label: 'Deliveries', value: profile.total_deliveries ?? 0, icon: Package, color: 'from-primary-500 to-primary-600' },
    { label: 'Hours Served', value: profile.total_hours ?? 0, icon: Clock, color: 'from-secondary-500 to-primary-500' },
    { label: 'Rating', value: profile.rating ?? 0, icon: Star, color: 'from-accent-500 to-accent-600' },
  ];
  ...
  return (
    ...
    >
      ...
      >
        <div className="absolute -top-12 -right-12 h-40 w-40 rounded-full bg-gradient-to-br from-primary-400/20 to-accent-400/20 blur-3xl" />
        <div className="relative">
          <div className="flex items-center gap-4 mb-6">
            <div className="h-20 w-20 rounded-2xl-premium bg-gradient-to-br from-primary-600 to-primary-500 text-white flex items-center justify-center text-2xl font-bold shadow-lg shadow-primary-600/30 shrink-0">
              {initials}
            </div>
            <div className="min-w-0">
              <h2 className="font-display text-xl font-bold text-ink dark:text-cream truncate">{profile.full_name}</h2>
              <span className="inline-flex items-center gap-1 mt-1 text-xs font-semibold text-primary-600 dark:text-primary-400 capitalize">
                <ShieldCheck className="h-3.5 w-3.5" /> {profile.role}
                {profile.is_verified && <Sparkles className="h-3.5 w-3.5 text-gold-500" />}
              </span>
            </div>
          </div>
          ...
          {profile.badges && profile.badges.length > 0 && (
            <div className="mt-6 pt-5 border-t border-gray-200/50 dark:border-gray-700/50">
              <p className="text-[11px] text-ink-soft dark:text-cream/50 uppercase tracking-wide mb-2">Badges Earned</p>
      ...
      <div className="lg:col-span-3 space-y-6">
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {stats.map((s) => (
            <motion.div key={s.label} variants={fadeInUp} whileHover={{ y: -4 }} className="glass-card p-5 relative overflow-hidden group">
              <div className={`absolute -top-6 -right-6 h-16 w-16 rounded-full bg-gradient-to-br ${s.color} opacity-10 blur-2xl group-hover:opacity-20 transition-opacity`} />
        ...
        >
          <div className="absolute -bottom-12 -left-12 h-40 w-40 rounded-full bg-gradient-to-br from-primary-500/10 to-accent-500/10 blur-3xl" />
          <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5">
            <div>
              <span className="eyebrow">Your dashboard</span>
              <h3 className="font-display text-xl font-bold text-ink dark:text-cream mt-2 capitalize">
        ...
        >
          <div className="h-12 w-12 rounded-xl bg-gradient-to-br from-secondary-500 to-primary-500 text-white flex items-center justify-center shrink-0 shadow-lg">
