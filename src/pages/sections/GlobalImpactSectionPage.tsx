...
function ZeroHungerTab() {
  return (
    <div className="space-y-12">
      ...
      <div>
        <SectionHeading badge="Hunger Challenge" title="Millions go hungry" subtitle="While food is wasted, millions have nothing to eat." />
        <div className="mt-14"><StatGrid stats={hungerStats} /></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="card p-8">
        <h3 className="font-display text-xl font-bold mb-6 text-ink dark:text-cream">CO2 Emissions: Food Waste vs. Nations</h3>
        <div className="space-y-4">
          {co2Data.map((d, i) => (
            <motion.div key={d.name} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="flex items-center gap-4">
              <span className="text-sm font-medium w-24 text-ink dark:text-cream">{d.name}</span>
              <div className="flex-1 h-6 rounded-full bg-oat dark:bg-secondary-800 overflow-hidden">
                <motion.div initial={{ width: 0 }} whileInView={{ width: `${(d.val / 12) * 100}%` }} viewport={{ once: true }} transition={{ duration: 1, delay: i * 0.1 }} className={`h-full ${d.color} rounded-full flex items-center justify-end pr-2`}>
                  <span className="text-xs font-bold text-white">{d.val} Gt</span>
                </motion.div>
              </div>
            </motion.div>
          ))}
        </div>
        <p className="text-sm text-ink-soft dark:text-cream/50 mt-6">If food waste were a country, it would be the third largest emitter of CO2 after China and the USA.</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="pt-20 min-h-screen gradient-bg-soft">
      ...
      <section className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto pb-16">
        <SectionTabs
          tabs={[
            { id: 'sdgs', label: 'UN SDGs', icon: Target, content: <SDGTab /> },
            { id: 'zero-hunger', label: 'Zero Hunger', icon: UtensilsCrossed, content: <ZeroHungerTab /> },
            { id: 'consumption', label: 'Responsible Consumption', icon: Recycle, content: <ConsumptionTab /> },
            { id: 'partnerships', label: 'Partnerships', icon: Handshake, content: <PartnershipsTab /> },
            { id: 'stats', label: 'Global Statistics', icon: BarChart3, content: <GlobalStatsTab /> },
            { id: 'map', label: 'World Impact Map', icon: MapPin, content: <WorldMapTab /> },
          ]}
        />
      </section>
    </div>
