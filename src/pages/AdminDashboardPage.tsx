...
import {
  ...
  TrendingDown, Minus, Radio, Building2, Store, Loader2,
} from 'lucide-react';
import {
...
const statIcons: Record<string, { icon: typeof Users; bg: string; spark: string }> = {
  ...
  total_donations: { icon: Package2, bg: 'from-primary-500 to-primary-600', spark: '#1B4332' },
  ...
  restaurants: { icon: Store, bg: 'from-rose-500 to-pink-500', spark: '#F43F5E' },
  ngos: { icon: Building2, bg: 'from-teal-500 to-green-500', spark: '#14B8A6' },
...
const emptyStats: OpsStat[] = Object.keys(statIcons).map((key) => ({
  key,
  label: '',
  value: 0,
  delta: 0,
...
export function AdminDashboardPage() {
  const [stats, setStats] = useState<OpsStat[]>(emptyStats);
  const [activities, setActivities] = useState<OpsActivity[]>([]);
  const [chartData, setChartData] = useState<{ label: string; users: number; volunteers: number; deliveries: number; waste: number }[]>([]);
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl glass">
              ...
              <span className="text-xs font-medium text-gray-600 dark:text-gray-300 flex items-center gap-1">
                {loading ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Live Data'}
              </span>
            </div>
          </div>
        </motion.div>
            ...
            return (
              ...
              >
                ...
                <p className="font-display text-xl sm:text-2xl font-bold tabular-nums">
                  {loading ? <Loader2 className="h-4 w-4 animate-spin text-gray-400" /> : <AnimatedCounter value={stat.value} suffix={stat.suffix ?? ''} />}
                </p>
              ...
              {!chartLoading && chartData.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={chartData} margin={{ top: 10, right: 10, bottom: 0, left: -10 }}>
                    ...
                    <Line type="monotone" dataKey="users" name="New Users" stroke="#3B82F6" strokeWidth={2.5} dot={{ r: 3 }} activeDot={{ r: 5 }} />
