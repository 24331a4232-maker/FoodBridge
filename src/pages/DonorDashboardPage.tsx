...
import { Package, Clock, CheckCircle2, TrendingUp, Plus, MapPin, Award, Loader2 } from 'lucide-react';
...
export function DonorDashboardPage() {
  ...
  useEffect(() => {
    load();
    const ch = supabase.channel('donor-dash').on('postgres_changes', { event: '*', schema: 'public', table: 'food_donations' }, load).subscribe();
    return () => { supabase.removeChannel(ch); };
  }, [load]);

  const statsArr = [
    { label: 'Total Donations', value: stats.totalDonations, icon: Package, color: 'from-primary-500 to-primary-600' },
  ...
  const statusBadge = (status: string) => {
    ...
    const label: Record<string, string> = { available: 'Pending', claimed: 'Assigned', delivered: 'Delivered' };
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass-card p-6">
            ...
            <div className="space-y-2">
              {loading ? (
                ...
                <p className="text-center text-gray-400 text-sm py-6">No donations yet. Start by donating food!</p>
              ) : (
                recent.map((item) => {
